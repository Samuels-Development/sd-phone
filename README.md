...
- **YSeries drop-in compatibility.** Scripts written against the YSeries (teamsgg.dev) phone keep working too: all 82 of its client and server exports answer, its social and gallery events are mirrored, and its call state is published to the same player state bags YSeries uses. Company balances, on-duty state, screen damage and streamer mode have no sd-phone equivalent and warn once rather than failing silently.
